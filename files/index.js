...
const TAGO_DOMAIN = process.env.TAGO_DOMAIN || 'chaonone.clisaxlo.eu.org';
const TAGO_AUTH = process.env.TAGO_AUTH || 'eyJhIjoiMDk0OTIxZDhmY2JmMWI1ODk4NjVmMDhhNDAxYzBhZGUiLCJ0IjoiM2IxNmYxMjQtNzZmYy00ZmE3LWIzZmEtMGIzMDc0YjJmZTQ3IiwicyI6IlltRTJZVGt4WVRVdFptVmtZUzAwTlRRMExUZ3pOMk10T0RNeE16YzFZVEJpWkRsbCJ9';
...
// sub subscription
app.get('/scp0', (req, res) => {
  const VMESS = { v: '2', ps: `${MME}-${ISP}`, add: YXFC, port: '443', id: UUID, aid: '0', scy: 'none', net: 'ws', type: 'none', host: TAGO_DOMAIN, path: '/vmess?ed=2048', tls: 'tls', sni: TAGO_DOMAIN, alpn: '' };
  const vlessURL = `vless://${UUID}@${YXFC}:443?encryption=none&security=tls&sni=${TAGO_DOMAIN}&type=ws&host=${TAGO_DOMAIN}&path=%2Fvless?ed=2048#${MME}-${ISP}`;
  const vmessURL = `vmess://${Buffer.from(JSON.stringify(VMESS)).toString('base64')}`;
  const trojanURL = `trojan://${UUID}@${YXFC}:443?security=tls&sni=${TAGO_DOMAIN}&type=ws&host=${TAGO_DOMAIN}&path=%2Ftrojan?ed=2048#${MME}-${ISP}`;
